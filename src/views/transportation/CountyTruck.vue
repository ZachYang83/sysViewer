...
let BASE_modeurl =
  "http://8.134.70.156:8090//tra_monitor/tra-rel-int/getOOnes/?code=";
let BASE_sumurl =
  "http://8.134.70.156:8090//tra_monitor/tra-rel-int/getOOnesByCity/?code=";
export default {
  data() {
    return {
      tripOptions: [
  ...
  methods: {
    changeMode(val) {
      ...
      switch (val) {
        case "o":
          ...
          BASE_sumurl =
            "http://8.134.70.156:8090//tra_monitor/tra-rel-int/getOOnesByCity/?code=";
          _this.pieTitle = "到各市货运联系强度占比";
          _this.barTitle = "货运联系强度前十地区";
          break;
        case "d":
          BASE_modeurl =
            "http://8.134.70.156:8090//tra_monitor/tra-rel-int/getDOnes/?code=";
          BASE_sumurl =
            "http://8.134.70.156:8090//tra_monitor/tra-rel-int/getDOnesByCity/?code=";
          _this.pieTitle = "各市到此货运联系强度占比";
          _this.barTitle = "货运联系强度前十地区";
          break;
      }
    },
    initLayers() {
      ...
      add_tms(window.MAP, "guangdong2", "fill", paint1);
      var paint2 = {
        "line-color": "#9e9e9e",
        "line-width": 1.1,
      };
    ...
    hoverLayer() {
      let _this = this;
      window.MAP.on("mousemove", "guangdong2",_this.onmousemove);
      window.MAP.on("mouseleave", "guangdong2", _this.onmouseleave);
    ...
    onclick(e) {
      ...
      features.reduce(
        function (memo, feature) {
          if (
            feature.sourceLayer == "guangdong2" &&
            feature.properties.CITY_CODE
          ) {
            var url = BASE_modeurl + feature.properties.CITY_CODE;
            ...
            var sumurl = BASE_sumurl + feature.properties.CITY_CODE;
      ...
      if (dataArray.length != 0) {
        let arcsDataCreator_line = function () {
          ...
          for (let i = 0; i < dataArray.length; i++) {
            ...
            const start = [parseFloat(olng), parseFloat(olon)];
            const end = [parseFloat(dlng), parseFloat(dlon)];
        ...
        let level1 = 50, level2=100, level3=300, level4=600, level5=1000, level6=100000;
    ...
    addBarChart() {
      ...
      this.$nextTick(() => {
        ...
        for (let i = barData.length - 1; i >= 0; i--) {
          ...
          if (mode == "o") {
            bar_datay.push(barData[i].d.substring(2));
          } else if (mode == "d") {
            bar_datay.push(barData[i].o.substring(2));
          }
        }
        var bar_option = {
          tooltip: {
  ...
  destroyed() {
    ...
    window.MAP.off("mouseleave", "guangdong2", _this.onmouseleave);
