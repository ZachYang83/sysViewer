...
import {
  ...
  getPubCityData,
  getPubDistData,
} from "api/publicInfo/eduInfo.js";
...
export default {
  ...
  methods: {
    ...
    setScatter() {
      get_eduData("/public_info/pub-edu/all").then((res) => {
    ...
    pie_chart() {
      var pie_Dom = document.getElementById("pie_edudata");
        ...
        series: [
          {
            ...
            data: [
              ...
              { value: 271, name: "文化" },
              { value: 416, name: "政法" },
              { value: 3482, name: "教育", selected: true },
            ],
          },
        ],
      };
    ...
    bar_chart() {
      var bar_Dom = document.getElementById("bar_edudata");
      var bar_Chart = echarts.init(bar_Dom);
      getPubDistData("/public_info/pubinfo-dist/edu_info").then((res) => {
        var pubDist_data = res.data.data;
        console.log(pubDist_data,'pubDist_data');
        let bar_datax = [];
        let bar_datay = [];
        for (let i = 0; i < pubDist_data.length; i++) {
          bar_datax.push(pubDist_data[i].sumEdu);
