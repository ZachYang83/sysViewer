...
export default {
  ...
  methods: {
    ...
    changeLayer(index) {
      ...
      window.MAP.removeLayer('landuse');
      window.MAP.removeSource('landuse');
      window.MAP.addSource("landuse", {
        type: "raster",
        tiles: [
          "http://8.134.70.156:8181/geoserver/gpzi/wms?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&FORMAT=image/png&TRANSPARENT=TRUE&STYLES&LAYERS=gpzi:ls"+ year +"&exceptions=application/vnd.ogc.se_inimage&SRS=EPSG:3857&WIDTH=769&HEIGHT=750&BBOX={bbox-epsg-3857}",
        ],
      });
      window.MAP.addLayer({
        id: "landuse",
  ...
  destroyed() {
    removeLayers(window.MAP, ["landuse"]);
  },
};
</script>
